...
class LoginFrame(wx.Frame):
    ...
    def __init__(self,parent,id,title,size):
        ...
        self.SetSize(size)
        self.Center()
        #StaticText：静态文本框 pos:position位置、方位；size：大小；lable：标签
        self.serverAddressLable = wx.StaticText(self,label='Server Address',pos=(10,50),size=(120,25))
        self.userNameLable = wx.StaticText(self,label='UserName',pos=(40,100),size=(120,25))
    ...
    def login(self,event):
        try:
            ...
            if response == b'UserName Empty':
                ...
                self.Close()
                ChatFrame(None, 2, title='WangShuo Chat Client', size=(500, 400))
        except Exception:
